...
public class TST {
	...
	private HashMap<String, String> map; // added a hashmap
	private int n; // size
	...
	private static class Node<String> {
		...
		private String val; // value associated with string //string over value
	}
	 ...
	 * @throws IllegalArgumentException if {@code key} is {@code null}
	 */
	public String get(String key) { // string over Value
		if (key == null) {
	 ...
	 * 
	 *         public Iterable<String> keysThatMatch(String pattern) { Queue<String>
	 *         queue = new Queue<String>(); collect(root, new StringBuilder(), 0,
	 *         pattern, queue); return queue; }
	 * 
	 *         private void collect(Node<String> x, StringBuilder prefix, int i,
	 *         String pattern, Queue<String> queue) { if (x == null) return; char c
	 *         = pattern.charAt(i); if (c == '.' || c < x.c) collect(x.left, prefix,
	 *         i, pattern, queue); if (c == '.' || c == x.c) { if (i ==
	 *         pattern.length() - 1 && x.val != null)
	 *         queue.enqueue(prefix.toString() + x.c); if (i < pattern.length() - 1)
	 *         { collect(x.mid, prefix.append(x.c), i+1, pattern, queue);
	 *         prefix.deleteCharAt(prefix.length() - 1); } } if (c == '.' || c >
	 *         x.c) collect(x.right, prefix, i, pattern, queue); }
	 */
	...
	public TST(String fileName) {
		...
		while (scanner.hasNextLine()) {
			...
			String[] informationBreakdown = information.split(",");// .trim().split("\\s+")
			String stopID = informationBreakdown[0];
			String informationBreakdown2 = informationBreakdown[2];
			String stopName = busStopFinder.alterStringName(informationBreakdown2, false);
			this.put(stopName, stopID); // tst
			String stopInfo = (" -------------------------------\n " + "ID:" + stopID + "\n Code:"
					+ informationBreakdown[1] + "\n Name:" + stopName + "\n Description:" + informationBreakdown[3]
					+ "\n Latitude:" + informationBreakdown[4] + "\n Longitude:" + informationBreakdown[5] + "\n Zone:"
					+ informationBreakdown[6] + "\n URL:" + informationBreakdown[7] + "\n Location Type:"
					+ informationBreakdown[8] + " \n Partent Station \n");
			map.put(stopID, stopInfo); // hashmap
	...
	public List<String> getStopInformation(String input) {
		...
		for (String x : keysWithPrefix(input)) {
			list.add(map.get(get(x)));
		}
		if (list.isEmpty()) {
			list.add("This input did not match any stops in our system.");
		}
		return list;
	}
